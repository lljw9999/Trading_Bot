...
class CryptoSystemValidator:
    ...
    def _test_coinbase_executor(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✅ Coinbase executor: account={bool(account)}, "
        ...
        try:
            ...
            for case in test_cases:
                ...
                if position_fraction <= 0.20:  # 20% max for crypto
                    valid_positions += 1
                    logger.info(
                        f"✅ {case['symbol']}: ${position_size:.0f} "
                        f"({position_fraction:.1%}) - {reason}"
                    )
                else:
                    logger.error(
                        f"❌ {case['symbol']}: position {position_fraction:.1%} "
                        f"exceeds 20% limit"
                    )
            ...
            for case in test_cases:
                ...
                if case["position"] > 15000:  # Above 15% limit
                    if max_allowed < Decimal(str(case["position"])):
                        passed_tests += 1
                        logger.info(
                            f"✅ {case['symbol']}: ${case['position']} limited to ${max_allowed:.0f} - {reason}"
                        )
                    else:
                        logger.error(
                            f"❌ {case['symbol']}: position not properly limited - {reason}"
                        )
                else:
                    if is_allowed:
                        passed_tests += 1
                        logger.info(
                            f"✅ {case['symbol']}: ${case['position']} allowed - {reason}"
                        )
                    else:
                        logger.error(
                            f"❌ {case['symbol']}: small position blocked - {reason}"
                        )
        ...
        try:
            ...
            for symbol in self.symbols:
                ...
                for i in range(30):
                    ...
                    if signal:
                        ...
                        logger.debug(
                            f"📊 {symbol}: edge={signal.edge_bps:.1f}bps, "
            ...
            if signals_generated > 0:
                ...
                test_signal = alpha.update_price(
                    "BTC-USD", 50500, "2025-01-15T10:30:00Z"
                )
                if test_signal:
            ...
            logger.info(
                f"✅ Momentum alpha: signals={signals_generated}, "
        ...
        try:
            ...
            for symbol in self.symbols:
                ...
                for i in range(35):  # More data points to generate signals
                    ...
                    alpha_signals_dict = {
                        "momentum_fast": (
                            alpha_signal.edge_bps,
                            alpha_signal.confidence,
                        )
                    }

                    ensemble_edge, ensemble_confidence = ensemble.predict(
                        alpha_signals=alpha_signals_dict, market_features=mock_features
                    )
            ...
            logger.info(
                f"✅ Integration pipeline: steps={pipeline_steps}, "
