...
from src.portfolio.signals import PortfolioSignalGenerator, PortfolioSignal, create_portfolio_generator
...
class TestSharpeOptimizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data = self._create_sample_data(n_days=150)  # Increased for more training data
    ...
    def test_model_training(self):
        ...
        self.assertLess(metrics['val_rmse'], 1.0)  # Should be much less than 1 for [0,1] targets
    ...
    def test_model_save_load(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.pkl') as tmp_file:
            ...
            self.assertEqual(new_optimizer.feature_columns, self.optimizer.feature_columns)
    ...
    def test_factory_function(self):
        ...
        optimizer = create_sharpe_optimizer(
            sharpe_window=15,
            max_position=0.3
        )
...
class TestPortfolioSignalGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i, asset in enumerate(self.assets):
            ...
            np.random.seed(42 + i)
            data = self._create_sample_data(asset, n_days=120)  # Increased for more training data
    ...
    def _create_sample_data(self, symbol: str, n_days: int = 120) -> pd.DataFrame:  # Increased default
    ...
    def test_portfolio_constraints_application(self):
        ...
        # Apply constraints
        portfolio_signals = self.generator.apply_portfolio_constraints(raw_signals, timestamp)
        
        self.assertIsInstance(portfolio_signals, dict)
        
        # Check total position doesn't exceed 100%
        total_position = sum(signal.position_size for signal in portfolio_signals.values())
        ...
        for signal in portfolio_signals.values():
            ...
            self.assertGreaterEqual(signal.confidence, self.generator.min_signal_strength)
    ...
    def test_correlation_penalty(self):
        ...
        penalty = self.generator._calculate_correlation_penalty('BTC', corr_matrix, signals)
