...
class TestModelRouter(unittest.TestCase):
    ...
    def _setup_redis_mock(self, symbol: str):
        ...
        def mock_hgetall(key):
            if key == f"instrument:info:{symbol}":
                return {
                    k.encode(): v.encode()
                    for k, v in self.instrument_data[symbol].items()
                }
            return {}
    ...
    def test_latency_requirement(self):
        ...
        self.assertLessEqual(
            avg_latency, 50.0, f"Average latency {avg_latency:.1f}µs exceeds 50µs"
        )
        self.assertLessEqual(
            max_latency, 100.0, f"Max latency {max_latency:.1f}µs too high"
        )
