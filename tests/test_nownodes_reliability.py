...
# Skip all NOWNodes tests unless USE_NOWNODES=1
USE_NOWNODES = os.getenv("USE_NOWNODES", "0") == "1"

if not USE_NOWNODES:
    pytest.skip("USE_NOWNODES=0 - NOWNodes tests disabled", allow_module_level=True)
...
@pytest.mark.nownodes
@pytest.mark.skipif(not USE_NOWNODES, reason="USE_NOWNODES=0 - NOWNodes tests disabled")
...
class TestConnectionReliability(unittest.TestCase):
    ...
    async def test_connection_retry_logic(self, mock_connect):
        ...
        result = await connect_with_retry(
            "wss://test.example.com", "BTCUSDT", max_retries=3
        )
    ...
    async def test_fallback_connection(self, mock_connect):
        ...
        result = await connect_with_retry(
            "wss://fallback.example.com", "ETHUSDT", use_fallback=True
        )
    ...
    async def test_connection_failure_after_retries(self, mock_connect):
        ...
        result = await connect_with_retry(
            "wss://test.example.com", "SOLUSDT", max_retries=3
        )
...
class TestReliabilityStats(unittest.TestCase):
    ...
    def test_reliability_calculation(self):
        ...
        success_rate = (
            CONNECTION_STATS["successes"] / CONNECTION_STATS["attempts"]
        ) * 100
...
class TestEndToEndReliability(unittest.TestCase):
    ...
    async def test_reliability_scenario_95_percent(self, mock_redis, mock_connect):
        ...
        for i in range(total_attempts):
            try:
                result = await connect_with_retry(
                    "wss://test.example.com", "TESTUSDT", max_retries=1
                )
                if result:
...
class TestConnectionReliabilitySync(AsyncTestCase):
    ...
    def test_successful_connection_sync(self):
        ...
        with patch(
            "src.layers.layer0_data_ingestion.nownodes_ws.websockets.connect"
        ) as mock_connect:
            mock_websocket = AsyncMock()
            mock_connect.return_value = mock_websocket

            result = self.run_async(
                connect_with_retry("wss://test.example.com", "BTCUSDT")
            )
    ...
    def test_reliability_scenario_sync(self):
        ...
        with patch(
            ...
            with patch(
                "src.layers.layer0_data_ingestion.nownodes_ws.REDIS"
            ) as mock_redis:
                mock_redis.publish = Mock()
                mock_websocket = AsyncMock()
                ...
                async def run_connections():
                    ...
                    for i in range(100):
                        task = connect_with_retry(
                            f"wss://test{i}.example.com", "TESTUSDT", max_retries=1
                        )
                        tasks.append(task)
