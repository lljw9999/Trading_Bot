...
class TestOrderBookPressureAlpha(unittest.TestCase):
    ...
    def test_generate_signal_buy_pressure(self):
        ...
        signal = self.alpha.generate_signal(
            symbol="BTC-USD", bid_size=10.0, ask_size=2.0, timestamp=self.timestamp
        )
    ...
    def test_generate_signal_sell_pressure(self):
        ...
        signal = self.alpha.generate_signal(
            symbol="ETH-USD", bid_size=3.0, ask_size=12.0, timestamp=self.timestamp
        )
    ...
    def test_generate_signal_balanced_book(self):
        ...
        signal = self.alpha.generate_signal(
            symbol="SOL-USD", bid_size=5.0, ask_size=5.0, timestamp=self.timestamp
        )
...
class TestMovingAverageMomentumAlpha(unittest.TestCase):
    ...
    def test_update_price_insufficient_data(self):
        ...
        for i in range(10):
            signal = self.alpha.update_price(
                "BTC-USD", 50000 + i, f"2025-01-15T10:{i:02d}:00Z"
            )
            self.assertIsNone(signal)
    ...
    def test_update_price_upward_momentum(self):
        ...
        for i in range(30):
            ...
            signal = self.alpha.update_price(
                "BTC-USD", price, f"2025-01-15T10:{i:02d}:00Z"
            )
            ...
            if i >= 29:  # Should generate signal on 30th update
                ...
                self.assertGreater(
                    signal.edge_bps, 0
                )  # Positive edge for upward momentum
    ...
    def test_update_price_downward_momentum(self):
        ...
        for i in range(30):
            ...
            if i >= 29:  # Should generate signal on 30th update
                ...
                self.assertLess(
                    signal.edge_bps, 0
                )  # Negative edge for downward momentum
                self.assertGreaterEqual(signal.confidence, 0.55)
                self.assertLessEqual(signal.confidence, 0.9)
...
class TestMetaLearnerLogisticBlending(unittest.TestCase):
    ...
    def test_can_use_logistic_blend(self):
        ...
        alpha_signals = {"ob_pressure": (15.0, 0.7), "ma_momentum": (20.0, 0.8)}
        self.assertTrue(self.meta_learner._can_use_logistic_blend(alpha_signals))

        # Test with missing signals
        alpha_signals = {"ob_pressure": (15.0, 0.7), "other_model": (10.0, 0.6)}
    ...
    def test_logistic_blend_mixed_signals(self):
        ...
        alpha_signals = {"ob_pressure": (15.0, 0.7), "ma_momentum": (-10.0, 0.6)}
    ...
    def test_predict_with_logistic_blend(self):
        ...
        edge, confidence = self.meta_learner.predict(
            alpha_signals, self.feature_snapshot
        )
...
class TestSyntheticSmokeRun(unittest.TestCase):
    ...
    def test_end_to_end_pipeline(self):
        ...
        obp_signal = obp_alpha.generate_signal(
            symbol=symbol, bid_size=10.0, ask_size=3.0, timestamp="2025-01-15T10:30:00Z"
        )
        ...
        if mam_signal and obp_signal:
            ...
            ensemble_edge, ensemble_confidence = meta_learner.predict(
            ...
            print(
                f"   MAM signal: {mam_signal.edge_bps:.1f}bps (conf: {mam_signal.confidence:.2f})"
            )
            print(
                f"   OBP signal: {obp_signal.edge_bps:.1f}bps (conf: {obp_signal.confidence:.2f})"
            )
            print(
                f"   Ensemble:   {ensemble_edge:.1f}bps (conf: {ensemble_confidence:.2f})"
            )
        else:
