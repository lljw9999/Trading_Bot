...
class MarketDataGenerator:
    ...
    def generate_btc_ticks(n_ticks: int = 10000) -> pd.DataFrame:
        ...
        timestamps = [start_time + timedelta(milliseconds=i*100) for i in range(n_ticks)]
        ...
        if model_type == 'tlob_tiny':
            ...
            if 'close' in data.columns:
                ...
                features = np.column_stack([
                    ...
                    price_features / np.convolve(price_features, np.ones(20)/20, mode='same')  # Price/MA20
                ])
...
class LatencyBenchmark:
    ...
    def _record_latency(self, model_name: str, symbol: str, data_type: str, 
    ...
    def benchmark_btc_ticks(self, n_iterations: int = 100) -> Dict:
        ...
        logger.info(f"🏁 Starting BTC tick latency benchmark ({n_iterations} iterations)")
        ...
        for model_name in ['tlob_tiny']:
            if not self.models_available.get(model_name, False):
                ...
                results[model_name] = self._simulate_latency(model_name, 'BTC-USD', 'ticks', n_iterations)
                continue
            ...
            try:
                ...
                for i in range(n_iterations):
                    ...
                    # Convert to features
                    features = self.data_generator.convert_to_features(sample_data, model_name)
                ...
                logger.info(f"✅ {model_name} BTC benchmark: {results[model_name]['p95_ms']:.2f}ms p95")
    ...
    def benchmark_nvda_bars(self, n_iterations: int = 100) -> Dict:
        ...
        logger.info(f"📊 Starting NVDA bar latency benchmark ({n_iterations} iterations)")
        ...
        for model_name in ['patchtst_small']:
            if not self.models_available.get(model_name, False):
                logger.warning(f"⚠️  Model {model_name} not available, using simulation")
                results[model_name] = self._simulate_latency(model_name, 'NVDA', 'bars', n_iterations)
                continue
            ...
            try:
                ...
                logger.info(f"✅ {model_name} NVDA benchmark: {results[model_name]['p95_ms']:.2f}ms p95")
    ...
    def _simulate_latency(self, model_name: str, symbol: str, data_type: str, 
    ...
    def print_results_table(self, results: Dict):
        ...
        for test_type, models in results.items():
            ...
            for model_name, model_results in models.items():
                ...
                print(f"{model_name:<20} {symbol:<10} {data_type:<10} "
        ...
        if PROMETHEUS_AVAILABLE and self.results:
            ...
            print(f"   - alpha_infer_latency_seconds_bucket: {len(self.results)} observations")
...
class TestLatencyBenchmarks:
    ...
    def test_btc_tick_latency(self, benchmark_suite):
        ...
        for model_name, model_results in results.items():
            assert model_results['p95_ms'] < 10.0, f"{model_name} P95 latency too high: {model_results['p95_ms']:.2f}ms"
            assert model_results['mean_ms'] < 5.0, f"{model_name} mean latency too high: {model_results['mean_ms']:.2f}ms"
