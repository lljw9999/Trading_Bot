...
from src.core.param_server import (
    ParamServer,
    ModelRoute,
    ModelRouterRules,
    create_param_server,
...
class TestParamServerCore(unittest.TestCase):
    ...
    def test_redis_publish_on_reload(self, mock_redis_class):
        ...
        param_server = ParamServer(
            str(self.config_path), redis_url="redis://localhost:6379/0"
        )
    ...
    def test_performance_benchmark(self):
        ...
        self.assertLess(
            avg_latency_us,
            50,
            f"get_rules() latency {avg_latency_us:.1f}µs exceeds 50µs target",
        )
        ...
        with self.assertRaises(Exception):
            rules_data = {
                "rules": [
                    ...
                    {
                        "match": {"asset_class": "stocks", "horizon_ms": "<60000"},
                        "model": "timesnet",
                        "priority": 10,
                    },
                ]
...
if __name__ == "__main__":
    ...
    suite.addTest(
        unittest.TestLoader().loadTestsFromTestCase(TestParamServerIntegration)
    )
