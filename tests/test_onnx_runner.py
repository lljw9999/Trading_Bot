...
pytest.importorskip(
    "onnxruntime", reason="onnxruntime optional dependency required for ONNX runner"
)
...
class TestONNXRunner(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patchtst_model_path = os.path.join(
            self.models_dir, "patchtst_small_int8.onnx"
        )
    ...
    def test_tlob_tiny_inference(self):
        ...
        self.assertTrue(
            -1.0 <= output[0, 0] <= 1.0, "Output should be in [-1, 1] range"
        )
    ...
    def test_model_benchmarking(self):
        ...
        self.assertLessEqual(
            stats["min_ms"], stats["median_ms"], "Min should be <= median"
        )
        self.assertLessEqual(
            stats["median_ms"], stats["max_ms"], "Median should be <= max"
        )
