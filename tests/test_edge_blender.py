...
from src.core.risk.edge_blender import EdgeBlender, ModelEdge, BlendedEdge, create_edge_blender
from src.core.risk.position_sizer import RiskAwarePositionSizer, PositionSizeResult, create_position_sizer
...
class TestEdgeBlender(unittest.TestCase):
    ...
    def test_decay_weight_calculation(self):
        ...
        model_edges = [
            ...
            ModelEdge("patchtst_small", 8.0, 0.7, time.time(), 300000), # λ=0.5 → w≈0.61
    ...
    def test_confidence_weighted_blending(self):
        ...
        model_edges = [
            ...
            ModelEdge("patchtst_small", -5.0, 0.4, time.time(), 300000) # Low confidence
        ]
    ...
    def test_confidence_threshold_filtering(self):
        ...
        model_edges = [
            ...
            ModelEdge("patchtst_small", 8.0, 0.2, time.time(), 300000), # Below threshold (0.3)
    ...
    def test_performance_benchmark_blend_latency(self):
        ...
        self.assertLess(avg_latency_us, 20, f"Blend latency {avg_latency_us:.1f}µs exceeds 20µs target")
...
class TestRiskAwarePositionSizer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_config = {
            "risk_harmoniser": {
                ...
                "monitoring": {
                    "performance": {"max_size_latency_us": 50}
                }
            }
        }
    ...
    def test_leverage_constraint_application(self):
        ...
        # Setup mock blender
        mock_blended_edge = BlendedEdge(
            symbol="BTC-USD", edge_blended_bps=15.0, edge_raw={}, weights={},
            confidences={}, kelly_frac=0.25, timestamp=time.time(),
            num_models=1, total_weight=1.0, shrinkage_applied=False
        )
        self.mock_edge_blender.blend_edges.return_value = mock_blended_edge
    ...
    def test_var_impact_calculation(self):
        ...
        mock_blended_edge = BlendedEdge(
            symbol="BTC-USD", edge_blended_bps=5.0, edge_raw={}, weights={},
            confidences={}, kelly_frac=0.10, timestamp=time.time(),
            num_models=1, total_weight=1.0, shrinkage_applied=False
        )
        self.mock_edge_blender.blend_edges.return_value = mock_blended_edge
    ...
    def test_asset_class_position_limits(self):
        ...
        mock_blended_edge = BlendedEdge(
            symbol="AAPL", edge_blended_bps=8.0, edge_raw={}, weights={},
            confidences={}, kelly_frac=0.30, timestamp=time.time(),
            num_models=1, total_weight=1.0, shrinkage_applied=False
        )
        self.mock_edge_blender.blend_edges.return_value = mock_blended_edge
    ...
    def test_performance_benchmark_sizing_latency(self):
        ...
        mock_blended_edge = BlendedEdge(
            symbol="BTC-USD", edge_blended_bps=7.0, edge_raw={}, weights={},
            confidences={}, kelly_frac=0.15, timestamp=time.time(),
            num_models=1, total_weight=1.0, shrinkage_applied=False
        )
        self.mock_edge_blender.blend_edges.return_value = mock_blended_edge
        ...
        for _ in range(10):
            position_sizer.calculate_position_size(
                "BTC-USD", model_edges, Decimal('50000'), 
                portfolio_value, "crypto"
            )
        ...
        self.assertLess(avg_latency_us, 50, f"Sizing latency {avg_latency_us:.1f}µs exceeds 50µs target")
    ...
    def test_redis_publishing_sized_position(self, mock_redis_class):
        ...
        mock_blended_edge = BlendedEdge(
            symbol="BTC-USD", edge_blended_bps=12.0, 
            edge_raw={"tlob_tiny": 12.0}, weights={"tlob_tiny": 1.0},
            confidences={"tlob_tiny": 0.8}, kelly_frac=0.20, 
            timestamp=time.time(), num_models=1, total_weight=1.0, 
            shrinkage_applied=False
        )
        self.mock_edge_blender.blend_edges.return_value = mock_blended_edge
        ...
        required_fields = ["symbol", "edge_blended_bps", "edge_raw", "weights", "kelly_frac", "size_usd"]
        for field in required_fields:
            self.assertIn(field, message_data)
...
class TestRiskHarmoniserIntegration(unittest.TestCase):
    ...
    def test_end_to_end_multi_model_blending(self):
        ...
        position_sizer = create_position_sizer(
            config_path=str(self.config_path),
            redis_url=None  # No Redis for unit test
        )
        ...
        self.assertGreater(result.target_position_usd, 0)  # Should be positive (positive dominant)
        self.assertEqual(result.blend_details.num_models, 3)  # All models should be included
        self.assertGreater(result.edge_blended_bps, 0)  # Blended edge should be positive
        self.assertLess(result.target_position_usd, portfolio_value * Decimal('0.20'))  # Crypto limit
    ...
    def test_memory_leak_detection(self):
        ...
        for i in range(100):  # Reduced for CI performance
            ...
            position_sizer.calculate_position_size(
                f"SYM-{i % 10}", varied_edges, Decimal('50000'), 
                portfolio_value, "crypto"
            )
        ...
        self.assertLess(memory_increase_mb, 20, f"Memory increase {memory_increase_mb:.1f}MB too high")
    ...
    def test_weights_sum_property(self):
        ...
        for model_edges in test_cases:
            ...
            if blended_edge.num_models > 0:
                ...
                self.assertGreaterEqual(blended_edge.edge_blended_bps, min_edge - 0.1)  # Small tolerance
...
if __name__ == '__main__':
    ...
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestRiskAwarePositionSizer))
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestRiskHarmoniserIntegration))
    suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestPropertyBasedBehavior))
