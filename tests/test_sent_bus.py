...
class TestSentimentBus(unittest.TestCase):
    ...
    def test_sentiment_lookup_within_window(self):
        ...
        asyncio.run(self.feature_bus.update_sentiment(
            self.test_symbol, 
            sentiment_score, 
            self.base_time
        ))
    ...
    def test_sentiment_lookup_no_data(self):
        ...
        result = self.feature_bus._get_latest_sentiment("UNKNOWN-SYMBOL", self.base_time)
        self.assertIsNone(result)
    ...
    def test_fundamental_lookup_within_window(self):
        ...
        pe_ratio = 25.5
        asyncio.run(self.feature_bus.update_fundamental(
            self.test_symbol,
            pe_ratio,
            self.base_time
        ))
...
class TestSentimentBusAsync(unittest.TestCase):
    ...
    def test_async_fundamental_update(self):
        ...
        async def run_test():
            ...
            # Verify data was stored
            result = self.feature_bus._get_latest_fundamental(self.test_symbol, timestamp)
            self.assertEqual(result, pe_ratio)
