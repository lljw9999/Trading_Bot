...
class StocksSession:
    ...
    def __init__(self):
        ...
        self.position_sizer = EnhancedPositionSizing(
            max_total_leverage=1.0,  # Conservative for stocks
            target_volatility=0.12,  # 12% target volatility for stocks
            lookback_days=252,
        )
    ...
    async def run_session(self, duration_minutes: int = 60):
        ...
        try:
            ...
            logger.info(
                f"📈 Trading {len(self.symbols)} symbols: {', '.join(self.symbols)}"
            )
            ...
            logger.info(
                f"💰 Starting portfolio value: ${float(self.portfolio_value):,.2f}"
            )
    ...
    async def _print_session_summary(self):
        ...
        try:
            ...
            duration = (
                datetime.now(timezone.utc) - self.connector.session_start
            ).total_seconds() / 60
            ...
            print(
                f"  Fill rate: {len(self.fills)/max(1,self.connector.total_signals):.1%}"
            )
            ...
            print(
                f"  Return: {(float(self.pnl)/float(self.connector.initial_portfolio)):.2%}"
            )
    ...
    async def _process_tick(self, tick: Dict):
        ...
        try:
            ...
            obp_signal = self.obp_alpha.update(
                symbol=symbol, bid_size=bid_size, ask_size=ask_size, timestamp=timestamp
            )
            ...
            if ensemble_edge:
                self.metrics.record_ensemble_prediction(
                    symbol=symbol, edge_bps=ensemble_edge.edge_bps
                )
            else:
                return

            # L3: Enhanced Position Sizing
            # Update market data for the enhanced system
            self.position_sizer.update_market_data(
                symbol=symbol,
                price_data={"close": price, "volume": 1000},  # Simplified for real-time
                returns=np.array(
                    [ensemble_edge.edge_bps / 10000]
                ),  # Convert bps to return
                features=np.array(
                    [ensemble_edge.confidence, ensemble_edge.edge_bps, price]
                ),
            )

            # Prepare signals for optimization
            alpha_signals = {symbol: ensemble_edge.edge_bps}
            confidence_scores = {symbol: ensemble_edge.confidence}
            ...
            if allowed_size != position_size:
                logger.info(
                    f"⚠️ Position size adjusted from ${position_size:,.2f} to ${allowed_size:,.2f}"
                )
    ...
    async def _execute_order(
        self, symbol: str, position_size: Decimal, price: float
    ) -> Optional[str]:
        ...
        try:
            ...
            if result and result.get("status") == "filled":
                ...
                new_pos = current_pos + (
                    position_size if side == "buy" else -position_size
                )
                ...
                self.metrics.update_portfolio_metrics(
                    ...
                    cash_balance=float(
                        ...
                        - sum(abs(p) for p in self.positions.values())
                    ),
                )
    ...
    async def _run_pipeline(self):
        ...
        try:
            while True:
                ...
                logger.info(
                    f"📈 Session stats: {elapsed:.1f}min, "
    ...
    async def _update_portfolio_value(self):
        ...
        try:
            ...
            drawdown = max(
                0,
                1
                - float(self.portfolio_value) / float(self.connector.initial_portfolio),
            )
            ...
            self.metrics.update_risk_metrics(
                risk_score=risk_score, drawdown=drawdown, exposure=exposure
            )
    ...
    async def _generate_session_report(self):
        ...
        try:
            ...
            # Alpha model stats
            alpha_stats = (
                self.obp_alpha.get_stats()
            )  # Assuming OBP is the primary alpha model for stats
            ...
            print(
                f"🎯 Signal-to-order ratio: {self.connector.total_orders/max(1, self.connector.total_signals):.2%}"
            )
            ...
            if self.connector.total_signals > 0:
                ...
                print(
                    f"  Avg signals/min: {self.connector.total_signals / elapsed:.1f}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--duration", type=int, default=60, help="Session duration in minutes"
    )
    parser.add_argument(
        "--metrics-port",
        type=int,
