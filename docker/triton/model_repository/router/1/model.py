...
class TritonPythonModel:
    ...
    def _load_onnx_models(self) -> None:
        ...
        try:
            ...
            for base_path in model_path_candidates:
                ...
                if os.path.exists(tlob_candidate) and os.path.exists(
                    patchtst_candidate
                ):
            ...
            self.tlob_session = ort.InferenceSession(
                tlob_path, sess_options=session_options
            )
            self.patchtst_session = ort.InferenceSession(
                patchtst_path, sess_options=session_options
            )

            self.logger.info(
                f"ONNX models loaded successfully from {tlob_path} and {patchtst_path}"
            )
    ...
    def _route_and_predict(
        self, symbol: str, features: np.ndarray, time_horizon: int
    ...
    def _select_model(
        self, symbol: str, time_horizon: int, feature_shape: Tuple
    ) -> str:
        """
        ...
        if (
            sequence_length > ROUTING_CONFIG["tlob_max_sequence"]
            or time_horizon > ROUTING_CONFIG["medium_horizon_threshold"]
        ):
            return "patchtst_small"

        # High-frequency symbols with short horizons → TLOB
        if (
            symbol_clean in ROUTING_CONFIG["hf_symbols"]
            and time_horizon <= ROUTING_CONFIG["short_horizon_threshold"]
        ):
            return "tlob_tiny"
    ...
    def _predict_tlob(self, features: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        try:
            ...
            if features.shape[1] > target_seq_len:
                ...
                padding = np.zeros(
                    (features.shape[0], pad_length, features.shape[2]), dtype=np.float32
                )
            ...
            if features.shape[2] != 10:
                if features.shape[2] > 10:
                    ...
                    padding = np.zeros(
                        (features.shape[0], features.shape[1], pad_features),
                        dtype=np.float32,
                    )
    ...
    def _create_response(
        self, model_choice: str, prediction: np.ndarray, confidence: float
    ) -> Any:
        ...
        model_choice_tensor = pb_utils.Tensor(
            "model_choice", np.array([model_choice], dtype=object)
        )
        ...
        return pb_utils.InferenceResponse(
            output_tensors=[model_choice_tensor, prediction_tensor, confidence_tensor]
        )
    ...
    def _update_stats(self, model_choice: str, latency_ms: float) -> None:
        ...
        if len(self.routing_stats["routing_latency_ms"]) > 1000:
            self.routing_stats["routing_latency_ms"] = self.routing_stats[
                "routing_latency_ms"
            ][-1000:]
        ...
        if total > 0:
            ...
            if self.routing_stats["routing_latency_ms"]:
                ...
                p95_latency = np.percentile(
                    self.routing_stats["routing_latency_ms"], 95
                )
            else:
