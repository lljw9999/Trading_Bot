...
async def test_components():
    ...
    for i in range(15):
        ...
        if signal:
            print(f"   Signal: edge={signal.edge_bps:.1f}bps, conf={signal.confidence:.2f}")
    
    # Test 2: Kelly sizing
    print("\n2. Testing Kelly Sizing...")
    kelly = KellySizing()
    pos, reason = kelly.calculate_position_size('AAPL', 10.0, 0.8, Decimal('150'), Decimal('100000'), 'stocks')
    ...
    allowed, risk_reason, max_allowed = risk_mgr.check_position_risk('AAPL', pos, Decimal('150'), Decimal('100000'))
    ...
    test_signal = alpha.update_price('AAPL', 148.0, '2025-01-15T10:20:00Z')  # Price drop
    if test_signal:
        print(f"   📊 Alpha signal: {test_signal.edge_bps:.1f}bps")
        
        # Kelly sizing
        pos, reason = kelly.calculate_position_size('AAPL', test_signal.edge_bps, test_signal.confidence, Decimal('148'), Decimal('100000'), 'stocks')
        print(f"   💰 Kelly position: ${pos:.0f}")
        
        # Risk check
        allowed, risk_reason, max_allowed = risk_mgr.check_position_risk('AAPL', pos, Decimal('148'), Decimal('100000'))
