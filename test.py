...
def get_args():
    ...
    p.add_argument("--symbol", default="BTC", choices=["BTC", "ETH", "SOL"],
                   help="asset (upper-case)")
    p.add_argument("--apikey", default=os.getenv("NOWNODES_APIKEY", ""),
                   help="NOWNodes API key (env:NOWNODES_APIKEY)")
    p.add_argument("--ticks", type=int, default=5, help="number of ticks to print before exit")
    p.add_argument("--timeout", type=int, default=10, help="max seconds to wait for first WSS msg")
    return p.parse_args()
...
async def main():
    ...
    try:
        ...
        print("✅  WSS test passed") ; sys.exit(0)
        ...
        print(f"⚠️   WSS failed ({type(e).__name__}: {e}) – falling back to REST polling…")

    # ----------- REST fallback -----------
    try:
        async for px, ts in run_rest(rest_uri, args.ticks):
            print(f"🔵 REST {args.symbol} {datetime.fromtimestamp(ts)}  ${px:,.2f}")
        print("✅  REST fallback passed") ; sys.exit(0)
