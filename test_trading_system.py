...
from src.layers.layer0_data_ingestion.schemas import (
    MarketTick,
    FeatureSnapshot,
    AlphaSignal,
    TradingDecision,
...
class TradingSystemTest:
    ...
    def __init__(self):
        ...
        self.alpha_models = {
            ...
            "ma_momentum": MovingAverageMomentumAlpha(),
        }
    ...
    async def test_layer0_feature_computation(self):
            ...
            for symbol in self.test_symbols[:2]:  # Test with crypto symbols
                ...
                if features:
                    ...
                    if i > 20:  # After some history is built
                        assert (
                            features.return_1m is not None
                            or features.volatility_5m is not None
                        )
        ...
        self.logger.info(
            f"✓ Layer 0: Computed {features_computed} features, avg time: {stats['avg_computation_time_us']:.1f}µs"
        )
    ...
    async def test_layer1_alpha_models(self):
        ...
        self.logger.info(
            f"✓ Layer 1: Generated signals from {len(alpha_signals)} models"
        )
        for model_name, (edge, conf) in alpha_signals.items():
            self.logger.info(
                f"  {model_name}: edge={edge:.2f}bps, confidence={conf:.3f}"
            )
    ...
    async def test_layer2_ensemble(self):
        ...
        # Create test signals
        alpha_signals = {"order_book_pressure": (5.2, 0.7), "ma_momentum": (-2.1, 0.6)}
        ...
        ensemble_edge, ensemble_confidence = self.meta_learner.predict(
        ...
        self.logger.info(
            f"✓ Layer 2: Ensemble edge={ensemble_edge:.2f}bps, confidence={ensemble_confidence:.3f}"
        )
    ...
    async def test_layer3_position_sizing(self):
        ...
        assert abs(position_size) <= portfolio_value * Decimal(
            "0.1"
        )  # Max 10% position
    ...
    async def test_layer4_execution(self):
        ...
        assert abs(position_value - target_position) < Decimal(
            "50"
        )  # Allow for slippage/commission
        ...
        self.logger.info(
            f"✓ Layer 4: Executed order, position=${position_value:.0f}, portfolio=${portfolio_value:.0f}"
        )
    ...
    async def test_layer5_risk_management(self):
        ...
        trading_allowed, trading_status = self.risk_manager.is_trading_allowed(
            risk_metrics
        )
        ...
        self.logger.info(
            f"✓ Layer 5: Risk score={risk_metrics.risk_score:.1f}, trading_allowed={trading_allowed}"
        )
    ...
    async def test_end_to_end_pipeline(self):
        ...
        self.logger.info(
            f"✓ End-to-end: Executed {trades_executed} trades, final portfolio=${portfolio_value:.0f}"
        )
    ...
    async def test_performance_metrics(self):
        ...
        stats = {
            ...
            "alpha_models": {
                name: model.get_stats() for name, model in self.alpha_models.items()
            },
        ...
        assert stats["feature_bus"][
            "performance_target_met"
        ]  # <300µs feature computation
        assert stats["executor"]["performance"]["total_trades"] > 0

        # Print summary
        self.logger.info("✓ Performance metrics collected:")
        self.logger.info(
            f"  Features computed: {stats['feature_bus']['features_computed']}"
        )
        self.logger.info(
            f"  Alpha predictions: {sum(m['prediction_count'] for m in stats['alpha_models'].values())}"
        )
        self.logger.info(
            f"  Ensemble predictions: {stats['meta_learner']['prediction_count']}"
        )
        self.logger.info(
            f"  Trades executed: {stats['executor']['performance']['total_trades']}"
        )
        self.logger.info(
            f"  Avg computation time: {stats['feature_bus']['avg_computation_time_us']:.1f}µs"
        )
