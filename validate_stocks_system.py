...
def validate_system():
    ...
    try:
        ...
        test_prices = [150.0, 151.0, 152.5, 154.0, 155.5, 157.0, 158.5, 160.0, 161.0, 162.0,
                      160.5, 159.0, 157.5, 156.0, 154.5, 153.0, 151.5, 150.5, 149.0, 148.0, 147.0, 146.0]
        signals_generated = 0
        ...
        for i, price in enumerate(test_prices):
            ...
            if signal:
                signals_generated += 1
                print(f"   Signal {signals_generated}: edge={signal.edge_bps:.1f}bps, conf={signal.confidence:.2f}")
        ...
        for edge_bps, confidence, scenario in test_scenarios:
            pos, reason = kelly.calculate_position_size(
                'AAPL', edge_bps, confidence, Decimal('150'), Decimal('100000'), 'stocks'
            )
        ...
        if kelly_tests_passed == len(test_scenarios):
            ...
            print(f"   ❌ Kelly sizing: {kelly_tests_passed}/{len(test_scenarios)} tests passed")
        ...
        for position_size, scenario in risk_tests:
            ...
            print(f"   {scenario}: {'✅ APPROVED' if allowed else '❌ REJECTED'} - {reason[:50]}...")
        ...
        integration_prices = [147.0, 146.5, 146.0, 145.5, 145.0, 144.5, 144.0, 143.5, 143.0, 142.5]
        test_signal = None
        
        for i, price in enumerate(integration_prices):
            test_signal = alpha.update_price('AAPL', price, f'2025-01-15T10:{30+i:02d}:00Z')
            if test_signal:
                break
        ...
        if test_signal:
            ...
            # Kelly sizing
            pos, reason = kelly.calculate_position_size(
                'AAPL', test_signal.edge_bps, test_signal.confidence, 
                Decimal('147'), Decimal('100000'), 'stocks'
            )
