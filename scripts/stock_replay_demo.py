...
class StockReplayDemo:
    ...
    def update_metrics(
        self, tick_data: dict, signals: dict, position_usd: float, pnl: float
    ) -> None:
        ...
        try:
            ...
            if "ensemble" in signals:
                ensemble_edge_url = (
                    f"{self.metrics_url}/metrics/gauge/alpha_signal_edge_bps"
                )
                ensemble_edge_data = {
    ...
    def process_tick(self, row: pd.Series) -> dict:
        ...
        mam_signal = self.mam.update_price(
            symbol=self.symbol, price=price, timestamp=tick_data["timestamp"]
        )
        ...
        # Update metrics
        self.update_metrics(
            tick_data, signals, self.position_usd, self.pnl_cumulative_usd
        )
    ...
    def run_replay(self, file_path: str):
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            if (
                i % 10 == 0
                or result["trade_result"]
                or result["risk_event"]
                or "ensemble" in result["signals"]
            ):
    ...
    def print_summary(self):
        ...
        if self.trades:
            ...
            for i, trade in enumerate(self.trades[-3:]):  # Show last 3 trades
                logging.info(
                    f"  {i+1}. {trade['shares']:+.0f} shares @ ${trade['price']:.2f} → ${trade['new_position_usd']:+.0f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--speed", type=float, default=30.0, help="Replay speed multiplier"
    )
