...
class CryptoReplayDemo:
        ...
        try:
            ...
            for signal_type, signal_data in signals.items():
                ...
                edge_data = {
                    ...
                    "labels": {"symbol": symbol, "model": signal_type},
                }
            ...
            if symbol in self.positions:
                ...
                pnl_data = {"value": self.pnl, "labels": {"symbol": symbol}}
    ...
    def process_tick(self, symbol: str, row: pd.Series) -> dict:
        ...
        # Simulate order book
        bid_price, ask_price, bid_size, ask_size = self.simulate_order_book(
            price, volume
        )
        ...
        if "obp" in signals and "mam" in signals:
            ...
            ensemble_conf = (
                prob
                * (signals["obp"]["confidence"] + signals["mam"]["confidence"])
                / 2.0
            )
    ...
    def print_tick_info(self, result: dict, tick_num: int, total_ticks: int):
        ...
        if signals_info:
            ...
            print(
                f"[{progress:5.1f}%] {result['symbol']} ${result['price']:.2f} | {signals_str}"
            )
    ...
    def print_summary(self):
        ...
        print(
            f"📊 Processed {self.tick_count:,} ticks in {elapsed:.1f}s ({ticks_per_sec:.1f} ticks/s)"
        )
        ...
        if self.positions:
            ...
            for symbol, pos in self.positions.items():
                if pos["size"] != 0:
                    print(
                        f"  {symbol}: Size=${pos['size']:+.0f}, Entry=${pos['entry_price']:.2f}, P&L=${pos['unrealized_pnl']:+.2f}"
                    )
...
def main():
    ...
    parser.add_argument(
        "data_dir",
        nargs="?",
        default="data/crypto/2025-07-02",
        help="Data directory path",
    )
    parser.add_argument(
        "--speed", type=float, default=5.0, help="Replay speed multiplier (default: 5x)"
    )
    parser.add_argument(
        "--symbols",
        nargs="+",
    ...
    parser.add_argument(
        "--metrics-url", default="http://localhost:8001", help="Metrics server URL"
    )
