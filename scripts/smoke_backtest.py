...
class SmokeBacktest:
    ...
    async def _process_tick(self, tick: MarketTick):
        ...
        try:
            ...
            self.metrics.record_feature_computation(tick.symbol, 100.0)  # 100μs simulated
            ...
            self.metrics.record_alpha_prediction('order_book_pressure', tick.symbol, edge1, conf1)
            
            edge2, conf2 = self.momentum_model.predict(features)
            alpha_signals['ma_momentum'] = (edge2, conf2)
            self.metrics.record_alpha_prediction('ma_momentum', tick.symbol, edge2, conf2)
            
            # Layer 2: Ensemble
            ensemble_edge, ensemble_conf = self.ensemble.predict(alpha_signals, features)
            ...
            trading_allowed, risk_reason = self.risk_manager.is_trading_allowed(risk_metrics)
                ...
                if order:
                    ...
                    if order.status.value == "filled":
                        self.metrics.record_order_filled(
                            tick.symbol, order.side.value, 0.1, 2.0  # 0.1s latency, 2bp slippage
                        )
            ...
            self.metrics.update_portfolio_metrics(float(portfolio_value), float(cash_balance))
    ...
    def _generate_results(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'portfolio_performance': {
                ...
                'final_positions': {k: float(v) for k, v in self.executor.positions.items()}
            },
            'component_stats': {
...
async def main():
    ...
    parser.add_argument('--date', help='Test date (YYYY-MM-DD) - ignored for synthetic data')
    ...
    try:
        ...
        print(f"   Performance: {results['portfolio_performance']['total_return_pct']:.2f}%")
