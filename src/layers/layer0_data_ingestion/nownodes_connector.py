...
class NOWNodesConnector:
    ...
    def __init__(
        self, api_key: str, endpoint: str, metrics_url: str = "http://localhost:8001"
    ):
        ...
        self.endpoint_map = {
            "BTC": os.getenv(
                "NOWNODES_BTC_WS",
                "wss://btc.blockbook.ws.nownodes.io/?api_key=" + api_key,
            ),
            "ETH": os.getenv(
                "NOWNODES_ETH_WS",
                "wss://eth.blockbook.ws.nownodes.io/?api_key=" + api_key,
            ),
            "SOL": os.getenv(
                "NOWNODES_SOL_WS",
                "wss://solana.blockbook.ws.nownodes.io/?api_key=" + api_key,
            ),
        }
    ...
    async def connect_symbol(self, symbol: str) -> None:
        ...
        if DEMO:
            logger.warning(
                f"Running in DEMO fake-price mode for {symbol} – NO real WS traffic"
            )
                ...
                async for raw_message in ws:
                    try:
                        ...
                        if "timestamp" in msg and "price" in msg:
                            ...
                            price = float(
                                msg.get("price", msg.get("bestBid", msg.get("last", 0)))
                            )
                        else:
                            # Fallback for different message formats
                            ts = datetime.now(timezone.utc)
                            price = float(
                                msg.get("bestBid", msg.get("price", msg.get("last", 0)))
                            )
    ...
    async def _process_tick(self, tick_data: Dict) -> None:
        ...
        if self.tick_counts[symbol] % 10 == 0:
            price_str = (
                f"${tick_data['price']:.2f}"
                if tick_data["price"] < 1000
                else f"${tick_data['price']:,.2f}"
            )
            logger.info(
                f"Processed {self.tick_counts[symbol]} ticks for {symbol} - Price: {price_str} (source: {tick_data['source']})"
            )
    ...
    def _update_metrics(self, symbol: str, tick_data: Dict) -> None:
        ...
        try:
            ...
            counter_data = {
                "labels": {"symbol": tick_data["symbol"], "source": tick_data["source"]}
            }
        ...
        try:
            while time.time() < end_time and self.running:
                ...
                if int(elapsed) % 60 == 0 and int(elapsed) > 0:
                    ...
                    logger.info(
                        f"Status: {elapsed/60:.1f}min elapsed, {total_ticks} total ticks"
                    )
...
def main():
    ...
    parser.add_argument(
        "--symbols", default="BTC,ETH,SOL", help="Comma-separated symbols"
    )
    parser.add_argument(
        "--duration-min", type=int, default=30, help="Duration in minutes"
    )
