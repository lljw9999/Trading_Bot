...
class IEXStockConnector(BaseDataConnector):
    ...
    def __init__(self, symbols: list[str], **kwargs):
        ...
        # Rate limiting
        self.rate_limit = config.get('data_ingestion.sources.stocks.iex.rate_limit', 100)
    ...
    async def _connect_impl(self) -> None:
        ...
        try:
            ...
            async with self.session.get(test_url, params={"token": self.api_key}) as response:
                if response.status == 200:
                    server_time = await response.json()
                    self.logger.info(f"Connected to IEX Cloud API, server time: {server_time}")
                else:
            ...
            for symbol in self.symbols:
                ...
                async with self.session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(f"Invalid symbol or API error for {symbol}: {response.status}")
    ...
    async def _parse_tick(self, raw_data: Dict[str, Any]) -> Optional[MarketTick]:
        ...
        try:
            ...
            return MarketTick(
                ...
                exchange_timestamp=datetime.fromtimestamp(quote.get("latestUpdate", 0) / 1000) if quote.get("latestUpdate") else None,
