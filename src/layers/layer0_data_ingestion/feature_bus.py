...
class FeatureBus:
    ...
    def __init__(self, max_history: int = 1000):
        ...
        self.fundamental_history: Dict[str, Deque[tuple[datetime, float]]] = defaultdict(
    ...
    async def update_sentiment(self, symbol: str, sentiment_score: float, timestamp: datetime = None):
    ...
    async def update_fundamental(self, symbol: str, pe_ratio: float, timestamp: datetime = None):
    ...
    def _get_latest_sentiment(self, symbol: str, reference_time: datetime) -> Optional[float]:
    ...
    def _get_latest_fundamental(self, symbol: str, reference_time: datetime) -> Optional[float]:
    ...
    async def process_tick(self, tick: MarketTick) -> Optional[FeatureSnapshot]:
        ...
        try:
            ...
            computation_time = (time.perf_counter() - start_time) * 1_000_000  # microseconds
    ...
    def _store_tick(self, tick: MarketTick) -> None:
        ...
        # Store price data
        if tick.mid:
            self.price_history[symbol].append((tick.timestamp, tick.mid))
        elif tick.last:
            self.price_history[symbol].append((tick.timestamp, tick.last))
        
        # Store volume data
        if tick.volume:
            self.volume_history[symbol].append((tick.timestamp, tick.volume))
    ...
    def _compute_order_book_imbalance(self, tick: MarketTick) -> Optional[float]:
        ...
        if total_size > 0:
            return float((tick.bid_size - tick.ask_size) / total_size)
    ...
    def _compute_order_book_pressure(self, tick: MarketTick) -> Optional[float]:
        ...
        bid_pressure = sum(float(price * size) for price, size in tick.bids[:5])
        ask_pressure = sum(float(price * size) for price, size in tick.asks[:5])
    ...
    def _compute_volume_ratio(self, volume_1m: Optional[Decimal], volume_5m: Optional[Decimal]) -> Optional[float]:
    ...
    async def _consume_loop(self):
        ...
        async for msg in self.consumer:
            ...
            if features:
                ...
                self.latency_hist.observe(latency_ms / 1000)  # seconds histogram uses seconds
    
    def _tick_from_json(self, data: Dict[str, Any]) -> Optional[MarketTick]:
        try:
            mid = (data["bid"] + data["ask"]) / 2 if data.get("bid") and data.get("ask") else None
            return MarketTick(
                symbol=data["symbol"],
                ...
                bid_size=Decimal(str(data["bid_size"])) if data.get("bid_size") else None,
                ask_size=Decimal(str(data["ask_size"])) if data.get("ask_size") else None,
                last=None,
                volume=None,
                metadata=data,
...
async def test_feature_bus():
    ...
    for i in range(100):
        ...
        if features:
            print(f"Tick {i}: mid={features.mid_price}, spread_bps={features.spread_bps}")
