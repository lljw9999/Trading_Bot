...
"""
import asyncio, json, os, time, logging, redis, random, ssl, warnings
from datetime import datetime, timezone
from dotenv import load_dotenv
from typing import Optional, Dict, List
import aiohttp

# Gate websockets imports behind USE_NOWNODES flag to avoid deprecation warnings
USE_NOWNODES = os.getenv("USE_NOWNODES", "0") == "1"

if not USE_NOWNODES:
    raise RuntimeError("NOWNODES disabled (set USE_NOWNODES=1)")

try:
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message=".*websockets.*deprecated.*")
        import websockets
    ...
    # Mock classes for when websockets is not available
    class MockWebSocketException(Exception):
        pass

    ConnectionClosed = MockWebSocketException
    InvalidStatusCode = MockWebSocketException
    WebSocketException = MockWebSocketException
...
SIMULATION_PRICES = {"BTCUSDT": 43000.0, "ETHUSDT": 2500.0, "SOLUSDT": 95.0}
...
async def connect_with_retry(
    url: str, symbol: str, max_retries: int = 3, use_fallback: bool = False
) -> Optional[object]:  # Changed from websockets.WebSocketServerProtocol to object
    """
    ...
    if not WEBSOCKETS_AVAILABLE:
        log.warning(
            f"⚠️  WebSockets not available for {symbol} - USE_NOWNODES=0 or websockets not installed"
        )
        return None
    ...
    for attempt in range(max_retries):
        ...
        try:
            log.info(
                f"🔄 Connecting to {symbol} via {'fallback' if use_fallback else 'primary'} (attempt {attempt + 1}/{max_retries})"
            )
            ...
            log.warning(
                f"⚠️  WebSocket connection failed for {symbol} (attempt {attempt + 1}): {e}"
            )

        except Exception as e:
            log.error(
                f"❌ Unexpected error connecting to {symbol} (attempt {attempt + 1}): {e}"
            )
...
async def stream_one_enhanced(symbol: str, primary_url: str):
        ...
        try:
            ...
            if fallback_url and not websocket:
                ...
                websocket = await connect_with_retry(
                    fallback_url, symbol, max_retries=2, use_fallback=True
                )
            ...
            log.warning(
                f"🔄 Reconnecting {symbol} in {delay}s (attempt {reconnect_count}/{max_reconnects})"
            )
            await asyncio.sleep(delay)
    ...
    log.warning(
        f"🎮 Falling back to simulation for {symbol} after {max_reconnects} failed reconnection attempts"
    )
...
def print_connection_stats():
    ...
    if total_attempts > 0:
        ...
        if success_rate >= 95:
            ...
            log.warning(
                f"⚠️  Below target reliability. Current: {success_rate:.1f}%, Target: ≥95%"
            )
...
async def main(symbols="BTCUSDT,ETHUSDT,SOLUSDT"):
    ...
    if not USE_NOWNODES:
        log.info("ℹ️  NOWNodes connector disabled - set USE_NOWNODES=1 to enable")
        log.info("🎮 Running in simulation-only mode")
        # Run simulation for all symbols
        tasks = []
        for sym in symbols.split(","):
            sym = sym.strip()
            tasks.append(asyncio.create_task(stream_simulation(sym)))
        if tasks:
    ...
    if not WEBSOCKETS_AVAILABLE:
        log.error(
            "❌ WebSockets library not available - install websockets or use simulation mode"
        )
        return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced NOWNodes WebSocket Connector"
    )
    parser.add_argument(
        "--symbols",
