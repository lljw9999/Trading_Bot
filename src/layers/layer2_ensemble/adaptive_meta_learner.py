...
class ConceptDriftDetector:
    ...
    def _check_drift(self):
        ...
        old_window = list(self.performance_history)[-self.window_size*2:-self.window_size]
...
class BayesianModelSelector:
    ...
    def __init__(self, alpha: float = 1.0, beta: float = 1.0):
        self.alpha = alpha
        self.beta = beta
        self.model_performance = defaultdict(lambda: {'successes': alpha, 'failures': beta})
    ...
    def select_models(self, available_models: List[str], n_models: int = 5) -> List[str]:
...
class AdaptiveMetaLearner:
    ...
    def detect_market_regime(self, recent_features: List[FeatureSnapshot]) -> MarketRegime:
    ...
    def update_model_performance(
        ...
        if pnl is not None:
            ...
            recent_pnls = [p.get('pnl', 0) for p in list(self.model_predictions[model_name])[-100:]]
            if len(recent_pnls) > 10:
                sharpe = np.mean(recent_pnls) / (np.std(recent_pnls) + 1e-8) * np.sqrt(252)
    ...
    def calculate_dynamic_weights(
        ...
        for i, model_name in enumerate(model_names):
            ...
            uncertainty_scores[i] = 1.0 - pred.uncertainty  # Lower uncertainty is better
        ...
        combined_scores = combined_scores * (1 - self.config.diversification_penalty * correlation_penalty)
    ...
    def _calculate_correlation_penalty(self, model_names: List[str]) -> np.ndarray:
        ...
        for model_name in model_names:
            recent_preds = list(self.model_predictions[model_name])[-50:]  # Last 50 predictions
            if recent_preds:
                preds_values = [p.get('prediction', 0) for p in recent_preds if isinstance(p, dict)]
        ...
        truncated_matrix = np.array([pred_list[:min_predictions] for pred_list in prediction_matrix])
        ...
        try:
            ...
            for i in range(n_models):
                other_correlations = [abs(correlation_matrix[i, j]) for j in range(n_models) if i != j]
                avg_correlations[i] = np.mean(other_correlations) if other_correlations else 0
    ...
    def combine_predictions(
        ...
        weights = self.calculate_dynamic_weights(filtered_predictions, self.current_regime)
        ...
        return AlphaSignal(
            ...
            symbol=self.symbol,
            timestamp=feature_snapshot.timestamp if feature_snapshot else datetime.now(),
    ...
    def get_ensemble_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'total_predictions': sum(p['total_predictions'] for p in self.model_performance.values()),
            'model_performance': {},
            'drift_detection': {},
            'ensemble_performance': {
                'recent_accuracy': np.mean(list(self.ensemble_performance)[-50:]) if self.ensemble_performance else 0,
        ...
        for model_name, perf in self.model_performance.items():
            ...
            stats['model_performance'][model_name] = {
                ...
                'drift_detected': self.drift_detectors.get(model_name, ConceptDriftDetector()).drift_detected,
            }
...
def create_adaptive_meta_learner(symbol: str, **kwargs) -> AdaptiveMetaLearner:
    ...
    config = EnsembleConfig(**{k: v for k, v in kwargs.items() if hasattr(EnsembleConfig, k)})
