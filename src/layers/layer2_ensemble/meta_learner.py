...
class MetaLearner:
    ...
    def __init__(self, model_name: str = "meta_learner"):
        ...
        # Load logistic blend weights
        self.logistic_weights = self._load_logistic_weights()
        
        self.logger.info(f"Meta-learner initialized with logistic blending")
    ...
    def predict(self, alpha_signals: Dict[str, Tuple[float, float]], 
                market_features: FeatureSnapshot) -> Tuple[float, float]:
        """
        ...
        """
        try:
            # Check if we have OBP + MAM signals for logistic blending
            if self._can_use_logistic_blend(alpha_signals):
                edge_bps, confidence = self._logistic_blend(alpha_signals)
                self.logger.debug(f"Using logistic blend: edge={edge_bps:.2f}bps, conf={confidence:.3f}")
            else:
    ...
    def _can_use_logistic_blend(self, alpha_signals: Dict[str, Tuple[float, float]]) -> bool:
    ...
    def _logistic_blend(self, alpha_signals: Dict[str, Tuple[float, float]]) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            # Adjust confidence based on input signal strengths
            avg_input_conf = (obp_conf + mam_conf) / 2.0
            blended_conf = blended_conf * avg_input_conf  # Scale by input confidence
            
            self.logger.debug(f"Logistic blend: obp={obp_edge:.1f}bp, mam={mam_edge:.1f}bp, "
    ...
    def _extract_features(self, alpha_signals: Dict[str, Tuple[float, float]], 
                         market_features: FeatureSnapshot) -> Optional[np.ndarray]:
    ...
    def _update_feature_names(self):
        ...
        for model_name in self.alpha_model_names:
            self.feature_names.extend([f"{model_name}_edge", f"{model_name}_confidence"])
        
        # Market context features
        self.feature_names.extend([
            "spread_bps", "volatility_5m", "volume_ratio", 
    ...
    def _simple_ensemble(self, alpha_signals: Dict[str, Tuple[float, float]]) -> Tuple[float, float]:
        ...
        try:
            ...
            if total_weight > 0:
                ...
                ensemble_confidence = np.mean([conf for _, conf in alpha_signals.values()])
            else:
                ensemble_edge = 0.0
    ...
    def _store_signal(self, alpha_signals: Dict[str, Tuple[float, float]], 
                     market_features: FeatureSnapshot,
                     ensemble_edge: float, ensemble_confidence: float):
        """Store signal for potential training."""
        try:
            signal_record = {
    ...
    def train(self, training_data: List[Tuple[Dict[str, Tuple[float, float]], 
                                           FeatureSnapshot, float]]) -> bool:
        """
        ...
        try:
            ...
            self.logger.info(f"Meta-learner training completed: "
    ...
    def load_model(self, filepath: str) -> bool:
        ...
        try:
            ...
            self.logistic_weights = model_data.get('logistic_weights', {'w1': 1.0, 'w2': 1.0})
            self.is_trained = True
