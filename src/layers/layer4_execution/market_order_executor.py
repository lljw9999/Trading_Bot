...
class MarketOrderExecutor:
    ...
    def __init__(
        ...
        # M16.1 optimization: venue-local tick nudging
        enable_tick_nudging: bool = True,
        tick_nudge_spread_threshold_bps: float = 6.0,  # Use mid+1tick when spread ≤6bp
    ):
        """
        ...
        Args:
            ...
            enable_tick_nudging: Whether to apply venue-local tick nudging
            tick_nudge_spread_threshold_bps: Spread threshold for tick nudging
        """
        self.commission_rate = commission_rate
        self.slippage_bps = slippage_bps
        self.paper_trading = paper_trading
        self.enable_tick_nudging = enable_tick_nudging
        self.tick_nudge_spread_threshold_bps = tick_nudge_spread_threshold_bps
    ...
    async def execute_order(
        ...
        try:
            ...
            if abs(position_delta) < min_trade_size:
                self.logger.debug(
                    f"No trade needed for {symbol}: delta=${position_delta:.2f}"
                )
                return None
            ...
            # Create order
            order = Order(
                symbol=symbol, side=side, quantity=shares_to_trade, order_type="market"
            )
    ...
    def apply_venue_tick_nudging(
        self,
        order: Order,
        market_price: Decimal,
        market_data: Optional[Dict[str, Any]] = None,
    ) -> Decimal:
        """Apply venue-local tick nudging optimization (M16.1)."""

        if not self.enable_tick_nudging or not market_data:
            return market_price

        # Get market data
        bid = market_data.get("bid", market_price)
        ask = market_data.get("ask", market_price)
        ...
        if bid > 0 and ask > 0:
            ...
            spread_bps = float("inf")  # No nudging for invalid data

        # Apply tick nudging if spread is tight enough
        if spread_bps <= self.tick_nudge_spread_threshold_bps:
            if order.side == OrderSide.BUY:
                # For buys, use mid + 1 tick
                nudged_price = mid_price + tick_size
            else:
                # For sells, use mid - 1 tick
                nudged_price = mid_price - tick_size

            self.logger.debug(
                f"Tick nudging: {order.symbol} {order.side.value} "
                f"spread={spread_bps:.1f}bps, price {market_price}->{nudged_price}"
            )
            return nudged_price
        else:
            # Spread too wide, use mid price
            if bid > 0 and ask > 0:
                return (bid + ask) / 2
            else:
                return market_price
    ...
    async def _simulate_execution(
        ...
        try:
            ...
            # Apply venue-local tick nudging (M16.1 optimization)
            adjusted_price = self.apply_venue_tick_nudging(
                order, market_price, market_data
            )

            # Calculate execution price with slippage
            slippage_factor = 1 + (self.slippage_bps / 10000)
            if order.side == OrderSide.BUY:
                execution_price = adjusted_price * Decimal(str(slippage_factor))
            else:
                execution_price = adjusted_price / Decimal(str(slippage_factor))
            ...
            self.positions[order.symbol] = (
                self.positions.get(order.symbol, Decimal("0")) + shares_delta
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_trades:
                avg_commission = sum(order.commission for order in recent_trades) / len(
                    recent_trades
                )
                total_volume = sum(
                    order.quantity * order.avg_fill_price for order in recent_trades
                )
                avg_slippage_bps = (
                    (self.total_slippage / total_volume) * 10000
                    if total_volume > 0
                    else 0
                )
            else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "positions": {
                symbol: float(shares) for symbol, shares in self.positions.items()
            },
