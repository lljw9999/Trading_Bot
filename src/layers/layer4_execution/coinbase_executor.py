...
class CoinbaseExecutor:
    ...
    def _validate_order(self, order: OrderRequest) -> bool:
        ...
        if order.order_type not in ['market', 'limit']:
            logger.error(f"❌ Order validation failed: Invalid order type '{order.order_type}'")
            return False
            
        if order.order_type == 'limit' and not order.limit_price:
            logger.error("❌ Order validation failed: Limit price required for limit orders")
            return False
    ...
    def submit_order(self, order: OrderRequest) -> OrderResponse:
        ...
        if self.dry_run or self.exec_mode == 'paper':
            ...
            order_json = {
                ...
                "order_configuration": {
                    "market_market_ioc" if order.order_type == 'market' else "limit_limit_gtc": {
                        "quote_size" if order.side == 'buy' else "base_size": str(order.qty)
                    }
                },
                "client_order_id": order.client_order_id
            }
            ...
            if order.order_type == 'limit':
                order_json["order_configuration"]["limit_limit_gtc"]["limit_price"] = str(order.limit_price)
            ...
            try:
                ...
                if cb_response.status_code == 200:
                    ...
                    logger.error(f"❌ Order submission failed: {cb_response.status_code} - {cb_response.text}")
                    response.status = 'rejected'
                    raise Exception(f"Order submission failed: {cb_response.status_code}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    order_response = OrderResponse(
                        ...
                        qty=float(order_data.get('order_configuration', {}).get('market_market_ioc', {}).get('quote_size', 0)),
                        status=order_data.get('status', 'unknown'),
                        filled_qty=float(order_data.get('filled_size', 0)),
                        avg_fill_price=float(order_data.get('average_filled_price', 0)) if order_data.get('average_filled_price') else None,
        ...
        if self.exec_mode == 'live':
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Find USD account
                    usd_account = next((acc for acc in accounts if acc.get('currency') == 'USD'), {})
...
def main():
    ...
    try:
        ...
        if order_status:
            logger.info(f"📋 Order Status: {json.dumps(asdict(order_status), indent=2)}")
