...
class KellySizing:
    ...
    def __init__(self, 
                 ...
                 min_edge_threshold: float = 1.0):  # Minimum 1bp edge
        """
    ...
    def calculate_position_size(self, 
                              ...
                              big_bet_flag: bool = False) -> Tuple[Decimal, str]:
        """
        ...
        try:
            ...
            if abs(edge_bps) < self.min_edge_threshold:
                return Decimal('0'), f"Edge {edge_bps:.1f}bps below threshold {self.min_edge_threshold}bps"
            
            # Check drawdown limits
            if self.current_drawdown > self.max_drawdown_limit:
                return Decimal('0'), f"Drawdown {self.current_drawdown:.1%} exceeds limit {self.max_drawdown_limit:.1%}"
            
            # Get instrument constraints
            constraints = self.instrument_constraints.get(instrument_type, self.instrument_constraints['crypto'])
            ...
            if big_bet_flag:
                ...
                max_var_position = self._calculate_max_var_position(portfolio_value, edge_bps, confidence)
                
                # Use minimum of 3x normal size or max VaR constraint
                big_bet_max = min(
                    self.max_position_size * big_bet_multiplier,
                    max_var_position
                )
                max_position_for_instrument = min(big_bet_max, constraints['max_position_pct'] * big_bet_multiplier)
                
                reasoning_prefix = f"BIG_BET(3x): "
            else:
                # Normal position sizing
                max_position_for_instrument = min(self.max_position_size, constraints['max_position_pct'])
                reasoning_prefix = ""
            ...
            if instrument_type == 'stocks':
                ...
                total_position_value = sum(abs(pos) for pos in self.current_positions.values())
                potential_total = total_position_value + (portfolio_value * Decimal(str(final_fraction)))
                ...
                if effective_leverage > max_leverage:
                    ...
                    max_additional = portfolio_value * (Decimal(str(max_leverage)) - 
                                                       Decimal(str(float(total_position_value / portfolio_value))))
                    final_fraction = min(final_fraction, float(max_additional / portfolio_value))
            ...
            self.logger.debug(f"Position size for {symbol}: ${position_dollars:.0f} ({reasoning})")
    ...
    def _calculate_kelly_fraction(self, edge_bps: float, confidence: float) -> float:
        ...
        try:
            ...
            p = max(0.01, min(0.99, confidence_adjusted_prob))  # Clamp to reasonable range
            q = 1 - p
    ...
    def get_position_limits(self, symbol: str) -> Dict[str, float]:
        ...
        return {
            ...
            'remaining_capacity': self.max_position_size - abs(self.current_positions.get(symbol, 0.0))
        }
    ...
    def _calculate_max_var_position(self, portfolio_value: Decimal, edge_bps: float, confidence: float) -> float:
        """
