...
class MomentumFastAlpha:
    ...
    def __init__(self, 
        ...
        logger.info(f"Momentum fast alpha initialized: fast={fast_period}, slow={slow_period}, "
    ...
    def update_price(self, symbol: str, price: float, timestamp: str) -> Optional[AlphaSignal]:
        ...
        try:
            ...
            if symbol not in self.price_history:
                ...
                self.ema_diff_history[symbol] = deque(maxlen=100)  # Keep history for z-score
    ...
    def _update_emas(self, symbol: str, price: float):
        ...
        try:
            # Update fast EMA
            self.fast_ema[symbol] = (price * self.fast_alpha) + (self.fast_ema[symbol] * (1 - self.fast_alpha))
            
            # Update slow EMA
            self.slow_ema[symbol] = (price * self.slow_alpha) + (self.slow_ema[symbol] * (1 - self.slow_alpha))
    ...
    def _generate_signal(self, symbol: str, timestamp: str) -> Optional[AlphaSignal]:
        ...
        try:
            ...
            confidence = min(1.0, confidence + ema_spread_pct * 100)  # Boost confidence for large spreads
            
            reasoning = (f"fast_ema={fast_ema:.2f}, slow_ema={slow_ema:.2f}, "
            ...
            logger.debug(f"Momentum signal for {symbol}: edge={edge_bps:.1f}bps, "
...
ALPHA_REGISTRY = {
    'momo_fast_crypto_v0': MomentumFastAlpha
}
...
def main():
    ...
    for symbol in symbols:
        ...
        for i in range(40):
            ...
            if signal:
                ...
                print(f"📊 {symbol} Signal {len(signals)}: edge={signal.edge_bps:.1f}bps "
        ...
        if signals:
            ...
            for signal in signals[:3]:
                realized_return = signal.edge_bps * 0.7  # 70% of predicted edge realized
    ...
    if len(all_signals) >= 2:
        strong_signals = [s for s in all_signals if abs(s.edge_bps) > 8.0]  # Strong signals
