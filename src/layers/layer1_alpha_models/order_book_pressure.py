...
class OrderBookPressure:
    ...
    def predict(self, features: FeatureSnapshot) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            if self.is_trained:
                ...
                edge_bps, confidence = self._heuristic_prediction(features)
    ...
    def _heuristic_prediction(self, features: FeatureSnapshot) -> Tuple[float, float]:
        ...
        try:
            ...
            spread_factor = 1.0 / (1.0 + (features.spread_bps or 10) / 10)  # Tighter spread = higher confidence
            vol_factor = 1.0 / (1.0 + (features.volatility_5m or 0.01) * 1000)  # Lower vol = higher confidence
