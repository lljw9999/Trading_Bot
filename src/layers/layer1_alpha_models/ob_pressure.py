...
class OrderBookPressureAlpha:
    ...
    def __init__(
        ...
        self.signal_count = 0
        self.hit_count = 0
        self.performance_updates = 0

        logger.info(
            f"Order book pressure alpha initialized: scaling={edge_scaling}, "
            f"min_conf={min_confidence}, max_conf={max_confidence}"
        )

    def generate_signal(
        self, symbol: str, bid_size: float, ask_size: float, timestamp: str
        ...
        try:
            ...
            logger.debug(
                f"OB pressure signal for {symbol}: edge={edge_bps:.1f}bps, "
    ...
    def update_from_feature_snapshot(self, feature_snapshot) -> Optional[AlphaSignal]:
        ...
        try:
            ...
            if (
                hasattr(feature_snapshot, "order_book_pressure")
                and feature_snapshot.order_book_pressure is not None
            ):
                ...
                hasattr(feature_snapshot, "order_book_imbalance")
                and feature_snapshot.order_book_imbalance is not None
            ):
                ...
                reasoning = (
                    f"ob_imbalance={imbalance:.4f}, proxy_pressure={pressure:.4f}"
                )
            ...
            logger.debug(
                f"OB pressure signal for {feature_snapshot.symbol}: edge={edge_bps:.1f}bps, "
    ...
    def predict(self, tick: Any) -> Tuple[float, float]:
        """Return edge/confidence for lightweight tick objects used in tests."""
        ...
        signal = self.generate_signal(symbol, bid_val, ask_val, timestamp_str)
        if signal is None:
            return 0.0, 0.0
    ...
    def get_hit_rate(self) -> float:
        ...
        if self.performance_updates == 0:
            return 0.5
        return self.hit_count / self.performance_updates
...
def main():
    ...
    for i, case in enumerate(test_cases):
        ...
        if signal:
            ...
            pressure = (case["bid_size"] - case["ask_size"]) / (
                case["bid_size"] + case["ask_size"] + 1e-9
            )

            print(
                f"📊 {case['symbol']} Signal: edge={signal.edge_bps:.1f}bps "
