...
class NewsSentimentAlpha:
    ...
    def __init__(self, 
        ...
        logger.info(f"News Sentiment Alpha initialized with edge_scaling={edge_scaling}bp")
    ...
    def update_from_feature_snapshot(self, feature_snapshot) -> Optional[AlphaSignal]:
        ...
        try:
            ...
            if not hasattr(feature_snapshot, 'sent_score') or feature_snapshot.sent_score is None:
            ...
            if sent_score != feature_snapshot.sent_score:
                logger.debug(f"Clipped sentiment score from {feature_snapshot.sent_score:.3f} to {sent_score:.3f} for {feature_snapshot.symbol}")
            ...
            strength_desc = "strong" if sentiment_strength >= self.strong_sentiment_threshold else "moderate"
            
            reasoning = f"sent_score={sent_score:.3f} ({strength_desc} {sentiment_desc})"
            ...
            logger.debug(f"News sentiment signal for {feature_snapshot.symbol}: "
                        f"edge={edge_bps:.1f}bp, confidence={confidence:.2f}, {reasoning}")
            
            return signal
            
        except Exception as e:
            logger.error(f"Error generating sentiment signal from feature snapshot: {e}")
            return None
    
    def update_price(self, symbol: str, price: float, timestamp: str, 
                     sentiment_score: float) -> Optional[AlphaSignal]:
        """
        ...
        try:
            ...
            if sent_score != sentiment_score:
                logger.debug(f"Clipped sentiment score from {sentiment_score:.3f} to {sent_score:.3f} for {symbol}")
            ...
            sentiment_strength = abs(sent_score)
            confidence = min(
                self.min_confidence + (sentiment_strength * 0.6) + 
                (0.2 if sentiment_strength >= self.strong_sentiment_threshold else 0.0),
                self.max_confidence
            )
            ...
            logger.debug(f"News sentiment signal for {symbol}: edge={edge_bps:.1f}bp, "
...
def test_sentiment_alpha():
    ...
    for sent_score, expected_edge, description in test_cases:
        ...
        if expected_edge is None:
            ...
            assert abs(signal.edge_bps - expected_edge) < 0.1, \
                f"Expected {expected_edge}bp, got {signal.edge_bps}bp for {description}"
            print(f"✅ {description}: edge={signal.edge_bps:.1f}bp, confidence={signal.confidence:.2f}")
