...
class MovingAverageMomentumAlpha:
    ...
    def __init__(self, 
        ...
        logger.info(f"MA momentum alpha initialized: short={short_period}, long={long_period}, "
    ...
    def _generate_signal(self, symbol: str, timestamp: str) -> Optional[AlphaSignal]:
        ...
        try:
            ...
            reasoning = (f"ma_short={ma_short:.2f}, ma_long={ma_long:.2f}, "
            ...
            logger.debug(f"MA momentum signal for {symbol}: edge={edge_bps:.1f}bps, "
    ...
    def update_from_feature_snapshot(self, feature_snapshot) -> Optional[AlphaSignal]:
        ...
        try:
            ...
            if hasattr(feature_snapshot, 'mid_price') and feature_snapshot.mid_price is not None:
...
def main():
    ...
    # Initialize model
    alpha = MovingAverageMomentumAlpha(short_period=5, long_period=30, edge_scaling=40.0)
    ...
    if conf_values:
        ...
        print(f"📊 Confidence in [0.55,0.9]: {all(0.55 <= c <= 0.9 for c in conf_values)}")
