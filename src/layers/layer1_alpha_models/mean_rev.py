...
class MeanReversionAlpha:
    ...
    def __init__(
        ...
        logger.info(
            f"Mean reversion alpha initialized: lookback={lookback_minutes}min, "
        ...
        try:
            ...
            if symbol not in self.price_history:
                self.price_history[symbol] = deque(
                    maxlen=self.lookback_minutes * 2
                )  # Buffer for returns calc
    ...
    def _generate_signal(self, symbol: str, timestamp: str) -> Optional[AlphaSignal]:
        ...
        try:
            ...
            current_return = sum(
                returns[-self.lookback_minutes :]
            )  # Sum of recent minute returns
            ...
            reasoning = (
                f"20min_return={current_return:.4f}, z_score={z_score:.2f}, "
            ...
            logger.debug(
                f"Mean reversion signal for {symbol}: edge={edge_bps:.1f}bps, "
...
ALPHA_REGISTRY = {"mean_rev_stock_v0": MeanReversionAlpha}
...
def main():
    ...
    for i, (timestamp, price) in enumerate(zip(timestamps, test_prices)):
        ...
        if signal:
            ...
            print(
                f"📊 Signal {len(signals)}: {signal.symbol} edge={signal.edge_bps:.1f}bps "
    ...
    if len(signals) >= 2:
        correct_predictions = sum(
            1 for s in signals if abs(s.edge_bps) > 5.0
        )  # Strong signals
