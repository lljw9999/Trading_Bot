...
class BigBetDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"Big-Bet Detector initialized: sent_thresh={sentiment_threshold}, "
    ...
    def _calculate_earnings_sigma(self, symbol: str) -> float:
        ...
        if (
            symbol not in self.earnings_history
            or len(self.earnings_history[symbol]) < 3
        ):
        ...
        try:
            ...
            if earnings_surprise is not None:
                ...
                if earnings_sigma > 0:
                    ...
                    earnings_extreme = (
                        earnings_sigma_multiple > self.earnings_sigma_threshold
                    )
            ...
            if big_bet_flag:
                ...
                earnings_component = np.sign(earnings_surprise) * min(
                    20, earnings_sigma_multiple * 10
                )  # Up to 20bp from earnings
                edge_bps = sentiment_component + earnings_component
                confidence = 0.85  # High confidence for big bets

                reasoning = (
                    f"BIG_BET: sent={sent_score:.3f} (>{self.sentiment_threshold}), "
                ...
                if earnings_surprise is not None:
                    reasoning = (
                        f"SENTIMENT: sent={sent_score:.3f} (>{self.sentiment_threshold}), "
                        f"earnings={earnings_sigma_multiple:.2f}σ (<{self.earnings_sigma_threshold}σ)"
                    )
                else:
                ...
                edge_bps = np.sign(earnings_surprise) * min(
                    25, earnings_sigma_multiple * 12
                )
                confidence = 0.5 + 0.3 * min(1.0, earnings_sigma_multiple / 2.0)

                reasoning = (
                    f"EARNINGS: sent={sent_score:.3f} (<={self.sentiment_threshold}), "
                    f"earnings={earnings_sigma_multiple:.2f}σ (>{self.earnings_sigma_threshold}σ)"
                )
                ...
                if earnings_surprise is not None:
                    reasoning = (
                        f"WEAK: sent={sent_score:.3f} (<={self.sentiment_threshold}), "
                        f"earnings={earnings_sigma_multiple:.2f}σ (<={self.earnings_sigma_threshold}σ)"
                    )
                else:
            ...
            logger.debug(
                f"Big-bet detector for {symbol}: flag={big_bet_flag}, "
    ...
    def update_from_feature_snapshot(
        self, feature_snapshot, earnings_surprise: Optional[float] = None
    ) -> BigBetSignal:
        """
    ...
    def update_performance(
        ...
        try:
            ...
            ) or (predicted_edge_bps < 0 and realized_return_bps < 0)
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        big_bet_hit_rate = (
            (self.big_bet_hits / self.big_bet_count) if self.big_bet_count > 0 else 0.0
        )
        big_bet_rate = (
            (self.big_bet_count / self.total_signals) if self.total_signals > 0 else 0.0
        )
...
def test_big_bet_detector():
    ...
    print(
        f"✅ Strong sentiment + earnings: flag={signal1.big_bet_flag}, edge={signal1.edge_bps:.1f}bp"
    )
    ...
    print(
        f"✅ Sentiment only: flag={signal2.big_bet_flag}, edge={signal2.edge_bps:.1f}bp"
    )
    ...
    print(
        f"✅ Weak signals: flag={signal3.big_bet_flag}, edge={signal3.edge_bps:.1f}bp"
    )
    ...
    print(
        f"✅ Negative big bet: flag={signal4.big_bet_flag}, edge={signal4.edge_bps:.1f}bp"
    )
