...
class PortfolioSignalGenerator:
    ...
    def __init__(self,
                 ...
                 lookback_days: int = 252):             # 1 year lookback for correlation
        """
    ...
    def calculate_risk_budgets(self) -> Dict[str, float]:
        ...
        risk_budgets = {symbol: inv_vol / total_inv_vol for symbol, inv_vol in inv_vols.items()}
        ...
        if total_budget > 0:
            risk_budgets = {symbol: budget / total_budget for symbol, budget in risk_budgets.items()}
    ...
    def generate_raw_signals(self, timestamp: datetime) -> Dict[str, Tuple[float, float]]:
        """
        ...
        for symbol, optimizer in self.optimizers.items():
            ...
            try:
                ...
                if len(position_sizes) > 0:
                    ...
                    if position_mean > 0:
                        confidence = max(0.1, min(0.9, 1.0 - (position_std / position_mean)))
                    else:
                        confidence = 0.5
    ...
    def apply_portfolio_constraints(self, 
                                  raw_signals: Dict[str, Tuple[float, float]],
                                  timestamp: datetime) -> Dict[str, PortfolioSignal]:
        """
        ...
        filtered_signals = {
            symbol: (pos, conf) for symbol, (pos, conf) in raw_signals.items()
        ...
        for other_symbol, (other_pos, _) in signals.items():
            ...
            if other_symbol in correlation_matrix.columns:
                ...
                if abs(correlation) > self.correlation_threshold:
                    penalty = 1.0 - (abs(correlation) - self.correlation_threshold) / (1.0 - self.correlation_threshold)
    ...
    def generate_signals(self, timestamp: datetime) -> List[PortfolioSignal]:
        ...
        for signal in signals_list:
            logger.info(f"  {signal.symbol}: {signal.position_size:.2%} ({signal.signal_type})")
    ...
    def get_portfolio_status(self) -> Dict:
        ...
        if len(self.current_positions) > 1 and self.correlation_matrix is not None:
            weights = np.array([self.current_positions.get(symbol, 0.0) 
                              for symbol in self.correlation_matrix.index])
            vols = np.array([self.volatility_estimates.get(symbol, 0.15) 
                           for symbol in self.correlation_matrix.index])
        ...
        return {
            ...
            'num_positions': len([p for p in self.current_positions.values() if p > 0.01]),
