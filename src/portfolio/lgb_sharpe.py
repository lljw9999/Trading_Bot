...
class SharpeOptimizer:
    ...
    def __init__(self,
                 ...
                 min_position: float = 0.01):   # Min 1% position size
        """
    ...
    def _add_momentum_features(self, df: pd.DataFrame) -> None:
        ...
        for window in self.momentum_windows:
            ...
            vwap = (df['volume'] * df['close']).rolling(window).sum() / df['volume'].rolling(window).sum()
    ...
    def calculate_target_sharpe(self, df: pd.DataFrame) -> pd.Series:
        ...
        rolling_mean = forward_returns.rolling(self.sharpe_window, min_periods=int(self.sharpe_window/2)).mean()
        rolling_std = forward_returns.rolling(self.sharpe_window, min_periods=int(self.sharpe_window/2)).std()
    ...
    def train(self, 
        ...
        if len(X) < 50:  # Need minimum samples for training
            raise ValueError(f"Insufficient training data: only {len(X)} valid samples. Need at least 50.")
