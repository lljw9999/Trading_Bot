...
class TimeSeriesWriter:
    ...
    def _connect_redis(self):
        ...
        try:
            ...
            try:
                ...
                self.has_timeseries = any(
                    "timeseries" in str(module).lower() for module in modules
                )
            except:
                self.has_timeseries = False
            ...
            if self.has_timeseries:
                ...
                logger.warning(
                    f"Connected to Redis at {self.redis_url} (TimeSeries module not available - using fallback)"
                )
    ...
    def _write_timeseries_metric(
        self, key: str, value: float, labels: Dict[str, str] = None
    ):
    ...
    def _write_fallback_metric(
        ...
        try:
            ...
            self.redis_client.zadd(
                ts_key, {json.dumps({"value": value, **labels}): timestamp}
            )
    ...
    def write_edge_blended(self, symbol: str, edge_bps: float, active_model: str):
        ...
        try:
            ...
            if success:
                ...
                logger.debug(
                    f"Wrote edge_blended_bps:{symbol} = {edge_bps:.2f} (model: {active_model})"
                )
            else:
    ...
    def write_risk_metrics_batch(
        ...
        try:
            ...
            if batch_success:
                logger.debug(
                    f"Batch wrote risk metrics for {symbol}: "
                    f"edge={edge_bps:.2f}bps, size=${size_usd:.0f}, var={var_pct:.2f}%"
                )
            else:
                logger.warning(
                    f"Batch write partially failed for {symbol}: {success_count}/3 metrics written"
                )
    ...
    def write_model_switch_event(
        self, symbol: str, old_model: str, new_model: str, latency_ms: float
    ):
        ...
        try:
            ...
            logger.info(
                f"Recorded model switch: {symbol} {old_model} → {new_model} ({latency_ms:.1f}ms)"
            )
            return True
    ...
    def get_timeseries_data(
        self, key: str, from_timestamp: int, to_timestamp: int
    ) -> List[tuple]:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self.writes_count / max(1, self.writes_count + self.failed_writes)
            )
            * 100,
...
def write_risk_metrics(
    symbol: str, edge_bps: float, size_usd: float, var_pct: float, active_model: str
) -> bool:
    """Convenience function to write risk metrics."""
    writer = get_timeseries_writer()
    return writer.write_risk_metrics_batch(
        symbol, edge_bps, size_usd, var_pct, active_model
    )
...
if __name__ == "__main__":
    ...
    for symbol in symbols:
        ...
        success = writer.write_risk_metrics_batch(
            symbol, edge_bps, size_usd, var_pct, model
        )
