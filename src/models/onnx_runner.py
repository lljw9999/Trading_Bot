...
# Optional ONNX dependency with gating
try:
    ...
    HAVE_ONNX = False
    ort = None
    # Re-raise the import error to match Future_instruction.txt pattern
    raise RuntimeError("Install onnx extras to use ONNX runner") from e
...
class ONNXRunner:
    ...
    def load_model(
        self, model_path: str, model_name: str, warm_up: bool = True
    ) -> bool:
        """
        ...
        try:
            with self._lock:
                ...
                logger.info(
                    f"   - Input: {metadata['input_name']} {metadata['input_shape']}"
                )
                logger.info(
                    f"   - Output: {metadata['output_name']} {metadata['output_shape']}"
                )
    ...
    def warm_up_model(self, model_name: str, num_iterations: int = 5) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Warming up model {model_name} with {num_iterations} iterations..."
            )
    ...
    def predict(
        self, model_name: str, input_data: np.ndarray, return_latency: bool = False
    ...
    def predict_batch(
        self, model_name: str, input_batch: List[np.ndarray]
    ) -> List[np.ndarray]:
        """
    ...
    def benchmark_model(
        self, model_name: str, num_iterations: int = 100
    ) -> Dict[str, float]:
        """
...
class ModelManager:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_info": {
                name: self.runner.get_model_info(name) for name in self.loaded_models
            },
        }
