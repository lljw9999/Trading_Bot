...
# Prometheus metrics
SENTIMENT_DOCS_TOTAL = Counter(
    "soft_docs_total", "Total sentiment documents processed", ["status"]
)
SENTIMENT_PROCESSING_TIME = Histogram(
    "sentiment_processing_seconds", "Time spent processing sentiment"
)
OPENAI_API_CALLS = Counter(
    "openai_api_calls_total", "Total OpenAI API calls", ["status"]
)
    ...
    while True:
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Enriched document for {enriched.symbol}: score={enriched.sentiment_score:.2f}"
                )
