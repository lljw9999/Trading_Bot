...
class TradingMetrics:
    ...
    def record_market_tick(self, symbol: str, exchange: str, asset_type: str, latency: float):
        """Record a market tick with latency."""
        self.market_ticks_received.labels(symbol=symbol, exchange=exchange, asset_type=asset_type).inc()
        self.market_tick_latency.labels(symbol=symbol, exchange=exchange).observe(latency)
    ...
    # Alpha model methods
    def record_alpha_prediction(self, model_name: str, symbol: str, edge_bps: float, confidence: float):
        """Record an alpha model prediction."""
        self.alpha_predictions.labels(model_name=model_name, symbol=symbol).inc()
        self.alpha_edge_bps.labels(model_name=model_name, symbol=symbol).observe(edge_bps)
        self.alpha_confidence.labels(model_name=model_name, symbol=symbol).observe(confidence)
    ...
    # Execution methods
    def record_order_submitted(self, symbol: str, side: str, order_type: str = "market"):
        """Record order submission."""
        self.orders_submitted.labels(symbol=symbol, side=side, order_type=order_type).inc()
    
    def record_order_filled(self, symbol: str, side: str, fill_latency: float, slippage_bps: float):
    ...
    def register_histogram(self, name: str, documentation: str, buckets: tuple[float, ...]):
