...
class EdgeBlender:
    ...
    def blend_edges(self, symbol: str, model_edges: List[ModelEdge]) -> BlendedEdge:
        ...
        try:
            ...
            # Apply confidence weighting and blend
            edge_blended_bps = self._blend_with_confidence(valid_edges, weights)
            
            # Apply Bayesian shrinkage if enabled
            if config.get('edge_blending', {}).get('shrinkage', {}).get('enabled', False):
                edge_blended_bps = self._apply_bayesian_shrinkage(
                    edge_blended_bps, valid_edges, config
                )
            ...
            kelly_frac = self._calculate_kelly_fraction(edge_blended_bps, symbol, config)
            ...
            # Performance check
            target_latency_us = config.get('monitoring', {}).get('performance', {}).get('max_blend_latency_us', 20)
            if blend_time_us > target_latency_us:
                logger.warning(f"Blend latency {blend_time_us:.1f}µs exceeds target {target_latency_us}µs")
            
            logger.debug(f"Blended {len(valid_edges)} models for {symbol}: "
    ...
    def _filter_valid_edges(self, model_edges: List[ModelEdge], config: Dict[str, Any]) -> List[ModelEdge]:
        ...
        valid_edges = [
            edge for edge in model_edges 
            if edge.confidence >= min_confidence
        ]
        
        # Limit number of models (take highest confidence ones)
        if len(valid_edges) > max_models:
            valid_edges = sorted(valid_edges, key=lambda x: x.confidence, reverse=True)[:max_models]
        
        return valid_edges
    
    def _calculate_decay_weights(self, model_edges: List[ModelEdge], config: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _blend_with_confidence(self, model_edges: List[ModelEdge], weights: Dict[str, float]) -> float:
        """
    ...
    def _apply_bayesian_shrinkage(self, edge_bps: float, model_edges: List[ModelEdge], config: Dict[str, Any]) -> float:
        """Apply Bayesian shrinkage towards prior edge."""
        shrinkage_config = config.get('edge_blending', {}).get('shrinkage', {})
        prior_edge = shrinkage_config.get('prior_edge_bps', 0.0)
        shrinkage_strength = shrinkage_config.get('shrinkage_strength', 0.1)
        
        # Shrink towards prior: edge_shrunk = (1 - α) * edge + α * prior
        shrunk_edge = (1 - shrinkage_strength) * edge_bps + shrinkage_strength * prior_edge
        
        return shrunk_edge
    
    def _calculate_kelly_fraction(self, edge_bps: float, symbol: str, config: Dict[str, Any]) -> float:
        """Calculate Kelly fraction for position sizing."""
    ...
    def _update_performance_metrics(self, model_edges: List[ModelEdge], blend_time_us: float):
        """Update performance tracking metrics."""
        self._blend_count += 1
        self._total_blend_time_us += blend_time_us
        
        for edge in model_edges:
            self._model_usage_stats[edge.model_id] = self._model_usage_stats.get(edge.model_id, 0) + 1
    ...
    def _load_config(self):
        ...
        try:
            ...
            logger.info(f"Loaded risk configuration: {len(config.get('edge_blending', {}).get('decay_factors', {}))} models configured")
...
def create_edge_blender(config_path: str = "conf/risk_params.yml", 
                       redis_url: Optional[str] = None) -> EdgeBlender:
