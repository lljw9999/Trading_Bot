...
class RiskAwarePositionSizer:
    ...
    def calculate_position_size(self,
                              ...
                              horizon_ms: int = 60000) -> PositionSizeResult:
        ...
        try:
            ...
            if blended_edge.num_models == 0:
                return self._create_zero_position(symbol, blended_edge, "No valid model edges")
            ...
            leverage_used = self._calculate_leverage_used(risk_adjusted_position, portfolio_value)
            ...
            logger.debug(f"Sized position for {symbol}: ${risk_adjusted_position:.0f} "
            ...
            zero_blend = BlendedEdge(
                symbol=symbol, edge_blended_bps=0.0, edge_raw={}, weights={}, 
                confidences={}, kelly_frac=0.0, timestamp=time.time(),
                num_models=0, total_weight=0.0, shrinkage_applied=False
            )
    ...
    def _apply_risk_constraints(self,
                              ...
                              asset_class: str,
                              portfolio_value: Decimal,
                              edge_bps: float) -> Tuple[Decimal, str]:
        ...
        max_position_pct = position_config.get('max_position_pct', {}).get(asset_class, 0.20)
        ...
        if abs(base_position) > max_position_usd:
            adjusted_position = max_position_usd if base_position > 0 else -max_position_usd
            return adjusted_position, f"Limited by {max_position_pct:.0%} asset class limit"
        
        # Apply leverage limit
        current_total_exposure = sum(abs(pos) for pos in self._current_positions.values())
    ...
    def _calculate_leverage_used(self, position_usd: Decimal, portfolio_value: Decimal) -> float:
        ...
        try:
            ...
            if TIMESERIES_AVAILABLE and result.edge_blended_bps != 0:
                ...
                if result.blend_details.weights:
                    active_model = max(result.blend_details.weights.items(), key=lambda x: x[1])[0]
                ...
                logger.debug(f"Wrote TimeSeries metrics for {result.symbol}: "
    ...
    def _create_zero_position(self, symbol: str, blend_details: BlendedEdge, reasoning: str) -> PositionSizeResult:
...
def create_position_sizer(config_path: str = "conf/risk_params.yml",
                         redis_url: Optional[str] = None,
                         edge_blender: Optional[EdgeBlender] = None) -> RiskAwarePositionSizer:
