...
class ParamServer:
    ...
    def __init__(self, config_path: str, redis_url: Optional[str] = None):
        ...
        logger.info(f"ParamServer initialized with {len(self._current_rules.rules) if self._current_rules else 0} rules")
    ...
    def _load_config(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Config loaded successfully: {len(new_rules.rules)} rules, "
    ...
    def _publish_reload_notification(self, event: ReloadEvent):
        ...
        try:
            ...
            if event.success and self._current_rules:
                config_dict = self._current_rules.dict()
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        with self._rules_lock:
            current_rules_count = len(self._current_rules.rules) if self._current_rules else 0
        ...
        return {
            ...
            "recent_reload_success_rate": successful_reloads / max(1, len(recent_events)),
            "recent_events": [
                {
    ...
    def validate_config_file(self, config_path: Optional[str] = None) -> tuple[bool, Optional[str]]:
...
def create_param_server(config_path: str, redis_url: Optional[str] = None) -> ParamServer:
