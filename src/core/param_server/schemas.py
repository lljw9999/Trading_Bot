...
class MatchCriteria(BaseModel):
    ...
    def matches_horizon(self, horizon_ms: int) -> bool:
        ...
        if "&" in horizon_str:
            ...
            return all(self._evaluate_single_condition(part, horizon_ms) for part in parts)
...
class ModelRoute(BaseModel):
    ...
    def matches(self, asset_class: str, horizon_ms: int) -> bool:
        ...
        return (
            self.match.matches_asset_class(asset_class) and
            self.match.matches_horizon(horizon_ms)
        )


class RouterConfig(BaseModel):
    """Router configuration settings."""
    default_model: str = Field("tlob_tiny", description="Default model when no rules match")
...
class ModelRouterRules(BaseModel):
    ...
    config: RouterConfig = Field(default_factory=RouterConfig, description="Router configuration")
    model_thresholds: Dict[str, ModelThreshold] = Field(
        default_factory=dict, description="Performance thresholds per model"
    )
    reload: ReloadConfig = Field(default_factory=ReloadConfig, description="Hot-reload settings")
    ...
    def find_matching_rule(self, asset_class: str, horizon_ms: int) -> Optional[ModelRoute]:
