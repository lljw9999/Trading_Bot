...
class RoutingRule:
    ...
    def matches(self, asset_class: AssetClass, horizon_ms: int) -> bool:
        ...
        return (
            self.asset_class == asset_class and
            self.horizon_min_ms <= horizon_ms <= self.horizon_max_ms
        )
...
class ModelRouter:
    ...
    def _get_instrument_info(self, symbol: str) -> Optional[InstrumentInfo]:
        ...
        if (symbol in self._instrument_cache and 
            ...
            current_time - self._cache_timestamps[symbol] < self.config.cache_ttl_seconds):
    ...
    def _infer_asset_class(self, symbol: str) -> AssetClass:
        ...
        if any(crypto in symbol_upper for crypto in ['BTC', 'ETH', 'SOL', 'USD', 'USDT']):
            return AssetClass.CRYPTO
        elif symbol_upper.endswith('.SS') or symbol_upper.endswith('.SZ'):
            return AssetClass.A_SHARES  
        elif any(stock_pattern in symbol_upper for stock_pattern in ['AAPL', 'MSFT', 'TSLA', 'GOOGL', 'AMZN']):
            ...
            for rule in self._compiled_rules:
                if rule.matches(instrument_info.asset_class, horizon_ms):
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(f"Selected {model_id} for {symbol} "
    ...
    def publish_selection(self, symbol: str, model_id: str, 
...
def create_model_router(redis_url: str = "redis://localhost:6379/0",
                       config_path: Optional[str] = None) -> ModelRouter:
