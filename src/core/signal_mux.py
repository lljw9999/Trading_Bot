...
class ModelRegistry:
    ...
    def register_model(
        self, model_id: str, model_instance: Any, metadata: Dict[str, Any]
    ):
        """Register a model with the registry."""
        self.models[model_id] = model_instance
        self.model_metadata[model_id] = metadata
        logger.info(
            f"Registered model {model_id}: {metadata.get('description', 'N/A')}"
        )
...
class SignalMux:
    ...
    async def process_tick(
        self, tick_data: TickData, horizon_ms: int
        ...
        try:
            ...
            if previous_model and previous_model != model_id:
                ...
                logger.info(
                    f"Model switch for {tick_data.symbol}: {previous_model} → {model_id}"
                )
            ...
            if not model:
                logger.warning(
                    f"Model {model_id} not found in registry for {tick_data.symbol}"
                )
                return None
    ...
    async def _prepare_features(
                ...
                features.update(
                    {
                        ...
                        "imbalance": (tick_data.bid_size - tick_data.ask_size)
        ...
        if self.feature_bus:
            try:
                fb_features = await self._get_feature_bus_data(
                    tick_data.symbol, horizon_ms
                )
                features.update(fb_features)
            except Exception as e:
                logger.debug(
                    f"Feature Bus integration failed for {tick_data.symbol}: {e}"
                )

        return features

    async def _get_feature_bus_data(
        self, symbol: str, horizon_ms: int
    ) -> Dict[str, Any]:
    ...
    async def _execute_model_prediction(
        ...
        try:
            ...
            if hasattr(model, "predict_async"):
                ...
                logger.warning(
                    f"Model {model_id} has no predict method, using dummy prediction"
                )
                return 0.0, 0.5
            ...
            )  # Re-raise to trigger process_tick failure
    ...
    async def _publish_tagged_signal(self, prediction: ModelPrediction):
        ...
        try:
            ...
            logger.debug(
                f"Published tagged signal for {prediction.symbol} via {prediction.model_id}"
            )
    ...
    async def _publish_switching_event(self, switching_event: Dict[str, Any]):
        ...
        try:
            ...
            if TIMESERIES_AVAILABLE:
                ...
                logger.debug(
                    f"Wrote model switch event to TimeSeries: "
                    f"{switching_event['symbol']} "
                    f"{switching_event['from_model']} → {switching_event['to_model']}"
                )

            logger.info(
                f"Published switching event: {switching_event['symbol']} "
                f"{switching_event['from_model']} → {switching_event['to_model']}"
            )
...
def create_signal_mux(
    ...
    for model_id, model_data in default_models.items():
        model_registry.register_model(
            model_id, model_data["instance"], model_data["metadata"]
        )
...
class DummyModel:
    ...
    def predict(self, features: Dict[str, Any]) -> tuple[float, float]:
        ...
        if "tlob" in self.model_id:
            ...
            edge_bps = (
                base_edge * 0.5
                + (hash(str(features.get("timestamp", 0))) % 100 - 50) * 0.1
            )
            confidence = 0.52
            ...
            edge_bps = (
                base_edge * 1.2
                + (hash(str(features.get("timestamp", 0))) % 100 - 50) * 0.2
            )
            confidence = 0.54
            ...
            edge_bps = (
                base_edge * 1.0
                + (hash(str(features.get("timestamp", 0))) % 100 - 50) * 0.15
            )
            confidence = 0.53
            ...
            edge_bps = (
                base_edge * 1.8
                + (hash(str(features.get("timestamp", 0))) % 100 - 50) * 0.3
            )
            confidence = 0.55
        else:
            # Chronos default
            edge_bps = (
                base_edge * 1.5
                + (hash(str(features.get("timestamp", 0))) % 100 - 50) * 0.25
            )
            confidence = 0.56
